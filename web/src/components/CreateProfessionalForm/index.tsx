...
export function CreateProfessionalForm({ form, defaultValues }: CreateProfessionalFormProps) {
  ...
  return (
    <div>
      {!professionalsType.length && (
        <Alert
          message="Informação"
          description="Antes de cadastrar um novo profissional, certifique-se de cadastrar antes o tipo do professional"
          type="warning"
          showIcon
        />
      )}
      <Form
        disabled={!professionalsType.length}
        layout="vertical"
        form={form}
